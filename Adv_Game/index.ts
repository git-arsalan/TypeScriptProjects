...
class Player
{
    name:string; // member variable as name
    fuel:number = 100; // member variable as fuel and assign initial value = 100
...
interface ac_type
{
    action_type: 'Attack'|'Energy Drink'| 'Quit'
}
...
async function startGame()
{
    try
    {
        const GamePlay : anstype = await inquirer.prompt(
            [
            {
             name: 'player1',
             type: 'input',
    ...
    await sleep();
           ...
           const action:ac_type  = await inquirer.prompt({
...
async function playGame(player:Player, opponent:opponent, action:string)
{
  // console.log(action);
    if (action == 'Attack')
    {
