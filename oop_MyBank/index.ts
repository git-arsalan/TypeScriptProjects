...
class Bank
{
    ...
    addCustomers(objCust:customer)
    {
        this.customers.push(objCust);

    }

    addAccounts(objAcc:Bankaccount)
    {
        this.accounts.push(objAcc);
    }

    performTransaction(accObj:Bankaccount) // This function used to update accounts after transaction
    {
...
for (let i:number=1; i<=3;i++) // Loop for adding customer and accounts in bank
{
...
async function availService (bank:Bank)
{
    while (true)
    {
    ...
    if (bankingService.serviceOpt == "View Balance")
    {
       // console.log("View Balance");
        ...
        {
            let cust = AbcBank.customers.find(
                (bank) => bank.accNum == acc_exists?.accNum );
